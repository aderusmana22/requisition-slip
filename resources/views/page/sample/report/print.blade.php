...
<head>
    ...
    <style>
        ...
        tfoot tr.signature-row td:nth-child(5) {
            ...
            /* turunkan posisi teks ke bawah */
            padding-top: 60px;
            /* tambahkan jarak agar sejajar dengan Approved */
        }
        ...
        .section-header td {
            ...
            border-right: none;
            border-top: none;
            background-color: #d6d0d0ff;
            /* Menambahkan background abu-abu seperti template */
        }
        ...
        .status-indicator {
            padding: 3px 10px;
            /* Padding diatur untuk bentuk pil */
            border-radius: 12px;
            /* Dibuat lebih bulat */
            font-size: 8pt;
            font-weight: bold;
        ...
        .status-review {
            border-color: #E8A903;
            /* Warna border kuning/oranye */
            color: #E8A903;
            /* Warna teks kuning/oranye */
        }
        ...
        .striped-table thead th {
            ...
            font-size: 10pt;
            /* Menambah ukuran font header */
            padding: 5px 3px;
            /* Menambah sedikit padding vertikal */
            text-transform: uppercase;
            /* Membuat teks menjadi kapital semua */
        }
        ...
        .striped-table td {
            padding: 4px 6px;
            word-wrap: break-word;
            /* INI SOLUSINYA: Memaksa teks untuk wrap */
            vertical-align: middle;
    ...
    @foreach ($reports as $index => $requisitions)
        ...
        @php
            $approvals = $requisitions->approvalLogs->map(function ($log) {
                ...
                if (
                    $log->status === 'Approved' &&
                    !empty($log->notes) &&
                    $log->notes !== 'Approved by ' . ($log->approver ? $log->approver->name : '')
                ) {
                ...
                // Get role display
                $roleNames =
                    $log->approver && $log->approver->roles ? $log->approver->roles->pluck('name')->toArray() : [];
            ...
            $requester = $requisitions->requester
                ? $requisitions->requester
                : (object) ['name' => 'Unknown', 'department' => (object) ['name' => 'Unknown']];
        @endphp
        <div class="page">
                        ...
                        <table class="no-border">
                            <tr>
                                <td style="width: 25%;">
                                    ...
                                    <span
                                        style="font-family: 'Times New Roman', Times, serif; font-size: 15pt; font-weight: bold; vertical-align: middle;">
                                        SINAR MEADOW
                                    </span>
                                </td>
                        ...
                        <table class="no-border" style="vertical-align: top;">
                            <tr>
                                <td style="width: 16%; vertical-align: top; padding-right: 30px;">
                                    ...
                                    <table class="no-border">
                                        <tr>
                                            <td style="text-align: left; padding-right: 5px;"><strong>CUSTOMER
                                                    NAME</strong></td>
                                            <td style="vertical-align: top; white-space: nowrap;">:
                                                {{ $requisitions->customer->name ?? '-' }}</td>
                                        </tr>
                                        <br>
                                        <tr>
                                            <td style="vertical-align: top; text-align: left; padding-right: 5px;">
                                                <strong>ADDRESS</strong></td>
                                            <td style="vertical-align: top; white-space: nowrap; width: 100%;">:
                                                {{ $requisitions->customer->address ?? '-' }}</td>
                                        </tr>
                                    </table>
                                </td>
                                <td style="width: 40%; vertical-align: top; padding-left: 450px;">
                                    ...
                                    <table class="no-border">
                                        <tr>
                                            <td style="text-align: right; padding-right: 5px;"><strong>Account</strong>
                                            </td>
                                            <td>: {{ $requisitions->account ?? '-' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="text-align: right; padding-right: 5px;"><strong>Cost
                                                    Center</strong></td>
                                            <td>: {{ $requisitions->cost_center ?? '-' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="text-align: right; padding-right: 5px;"><strong>Tanggal</strong>
                                            </td>
                                            <td>:
                                                {{ \Carbon\Carbon::parse($requisitions->request_date)->format('d F Y') }}
                                            </td>
                                        </tr>
                                        <br>
                                        <tr>
                                            <td style="text-align: right; padding-right: 5px;"><strong>Nomor RS</strong>
                                            </td>
                                            <td>: <strong style="font-size: 14pt;">{{ $requisitions->no_srs }}</strong>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                @foreach ($requisitions->requisitionItems as $item)
                                    <tr>
                                        <td class="text-center">
                                            {{ $item->itemMaster->unit ?? ($item->itemDetail->unit ?? '-') }}</td>
                                        ...
                                        <td class="text-center">
                                            {{ $item->quantity_issued > 0 ? $item->quantity_issued : '' }}</td>

                                        @if ($loop->first)
                                            <td class="notes-column text-center" rowspan="{{ $totalRows }}">
                                                {{ $requisitions->objectives }}</td>
                                            @if (!$isRndOrQa)
                                                <td class="notes-column text-center" rowspan="{{ $totalRows }}">
                                                    {{ $requisitions->estimated_potential }}</td>
                                            @endif
                                        @endif
                                    </tr>
                                @endforeach
        ...
        @if ($requisitions->sub_category === 'Special Order' && $requisitions->requisitionSpecial)
            @php
                ...
                $is_other_packaging =
                    $special->packaging_selection && !in_array($special->packaging_selection, $packaging_options);
                $is_other_shipment =
                    $special->shipment_method && !in_array($special->shipment_method, $shipment_options);
                $is_other_source = $special->source && !in_array($special->source, $source_options);
                $is_other_preparation =
                    $special->preparation_method && !in_array($special->preparation_method, $preparation_options);
            ...
            <div class="page page-break">
                <table class="outer">
                    ...
                    <tr>
                        <td colspan="6"
                            style="text-align: center; font-weight: bold; font-size: 14px; padding: 4px 8px;">
                    ...
                    <tr style="background-color: #e4e4e4ff;">
                        <td colspan="6" class="text-center" style="font-weight: bold; font-size: 16px;">PERMINTAAN
                            SAMPLE</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td
                            style="width: 20%; white-space: nowrap; border: none; border-right: 1px solid #333; padding-right: 8px;">
                            Tgl Permintaan</td>
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="5">:
                            {{ \Carbon\Carbon::parse($requisitions->request_date)->locale('id_ID')->isoFormat('D MMMM YYYY') }}
                        </td>
                        <td style="white-space: nowrap; border: none;">No. SRS : <strong
                                style="font-size: 14px;">{{ $requisitions->no_srs }}</strong></td>
                    </tr>
                    <tr>
                        <td
                            style="width: 20%; white-space: nowrap; border: none; border-right: 1px solid #333; padding-right: 8px;">
                            Tgl Selesai Sample</td>
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">:
                            {{ \Carbon\Carbon::parse($special->end_date)->locale('id_ID')->isoFormat('D MMMM YYYY') }}
                        </td>
                    </tr>
                    <tr>
                        <td
                            ...
                            Produk</td>
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">:
                            <strong>{{ $special->products }}</strong></td>
                    </tr>
                    <tr>
                        <td
                            style="width: 20%; white-space: nowrap; border: none; border-right: 1px solid #333; padding-right: 8px;">
                        ...
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            ...
                            @foreach (['a. 25Kg', 'b. 15Kg', 'c. 250gr', 'd. 500gr', 'e. 1Kg', 'f. 500ml', 'g. 1lt', 'h. 5lt'] as $option)
                                @if (str_contains($option, $special->weight_selection) && !$is_other_weight)
                                    <b><u>{{ $option }}</u></b>@else{{ $option }}
                                @endif
                                &nbsp;&nbsp;&nbsp;
                            @endforeach
                            i. Lainnya: @if ($is_other_weight)
                            <b><u>{{ $special->weight_selection }}</u></b>@else.............
                            @endif
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            ...
                            @foreach (['a. Tub', 'b. Karton', 'c. Botol', 'd. Jerrycan'] as $option)
                                @if (str_contains($option, $special->packaging_selection) && !$is_other_packaging)
                                    <b><u>{{ $option }}</u></b>@else{{ $option }}
                                @endif
                                &nbsp;&nbsp;&nbsp;
                            @endforeach
                            e. Lainnya: @if ($is_other_packaging)
                            <b><u>{{ $special->packaging_selection }}</u></b>@else.............
                            @endif
                        </td>
                    </tr>
                    <tr>
                        <td
                            ...
                            Jumlah sample</td>
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">:
                            <strong>{{ $special->sample_count }}</strong></td>
                    </tr>
                    <tr>
                        <td
                            style="width: 20%; white-space: nowrap; border: none; border-right: 1px solid #333; padding-right: 8px;">
                            Tujuan sample</td>
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">:
                            <strong>{{ $special->purpose }}</strong></td>
                    </tr>
                    <tr>
                        <td
                            style="width: 20%; white-space: nowrap; border: none; border-right: 1px solid #333; padding-right: 8px;">
                    ...
                    <tr>
                        ...
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            ...
                            @foreach (['a. Sales', 'b. Delivery (DHL)', 'c. Container', 'd. Kurir'] as $option)
                                @if (str_contains($option, $special->shipment_method) && !$is_other_shipment)
                                    <b><u>{{ $option }}</u></b>@else{{ $option }}
                                @endif
                                &nbsp;&nbsp;&nbsp;
                            @endforeach
                            e. Lainnya: @if ($is_other_shipment)
                            <b><u>{{ $special->shipment_method }}</u></b>@else.............
                            @endif
                        </td>
                    </tr>
                    ...
                    {{-- BAGIAN QA/QM --}}
                    <tr class="section-header">
                        <td colspan="7" class="text-center" style="font-style: italic; font-weight: bold;">Diisi
                            oleh QA</td>
                    </tr>
                    <tr>
                        <td
                        ...
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            :
                            @if ($special->source == 'WH')
                                <b><u>a. WH</u></b>
                            @else
                                a. WH
                            @endif&nbsp;&nbsp;&nbsp;
                            @if ($special->source == 'Reference Sample')
                                <b><u>b. Reference Sample</u></b>
                            @else
                                b. Reference Sample
                            @endif&nbsp;&nbsp;&nbsp;
                            @if ($special->source == 'Batch Refinery')
                                <b><u>c. Batch Refinery</u></b>
                            @else
                                c. Batch Refinery
                            @endif&nbsp;&nbsp;&nbsp;
                            @if ($special->source == 'Packing Room')
                                <b><u>d. Packing Room</u></b>
                            @else
                                d. Packing Room
                            @endif&nbsp;&nbsp;&nbsp;
                            e. Lainnya: @if ($is_other_source)
                            <b><u>{{ $special->source }}</u></b>@else.............
                            @endif&nbsp;&nbsp;&nbsp;
                        </td>
                    </tr>
                    <tr>
                        <td
                            style="width: 20%; white-space: nowrap; border: none; border-right: 1px solid #333; padding-right: 8px;">
                            Keterangan sample</td>
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            : a. Batch / Pallet No: {!! $batch_no !!} P {!! $pallet_no !!} &nbsp;&nbsp; b.
                            WB/DEO No: {!! $wb_deo_no !!} &nbsp;&nbsp; c. Tank No: {!! $tank_no !!}
                        </td>
                    </tr>
                    <tr>
                        <td
                    ...
                    <tr>
                        ...
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            :
                            @if ($special->preparation_method == 'Tidak berubah')
                                <b><u>a. Tidak berubah</u></b>
                            @else
                                a. Tidak berubah
                            @endif&nbsp;&nbsp;&nbsp;
                            @if ($special->preparation_method == 'Rework Karton')
                                <b><u>b. Rework Karton</u></b>
                            @else
                                b. Rework Karton
                            @endif&nbsp;&nbsp;&nbsp;
                            @if ($special->preparation_method == 'Rework Stencill')
                                <b><u>c. Rework Stencil</u></b>
                            @else
                                c. Rework Stencil
                            @endif&nbsp;&nbsp;&nbsp;
                            @if ($special->preparation_method == 'Rework Label')
                                <b><u>d. Rework Label</u></b>
                            @else
                                d. Rework Label
                            @endif&nbsp;&nbsp;&nbsp;
                            e. Lainnya: @if ($is_other_preparation)
                            <b><u>{{ $special->preparation_method }}</u></b>@else.............
                            @endif&nbsp;&nbsp;&nbsp;
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="white-space: nowrap; border: none; padding-left: 8px;" colspan="6">
                            :
                            @if ($special->sample_notes == 'Tempel sticker')
                                <b><u>a. Tempel sticker</u></b>
                            @else
                                a. Tempel sticker
                            @endif&nbsp;&nbsp;&nbsp;
                            b. Lainnya: @if ($is_other_notes)
                            <b><u>{{ $special->sample_notes }}</u></b>@else.............
                            @endif
                        </td>
                    </tr>
                </table>
                    ...
                    <td style="padding: 10px;">
                        ...
                        <table class="no-border" style="margin-bottom: 20px;">
                            <tr>
                                ...
                                <td style="width: 60%;" class="text-center">
                                    <div class="font-bold main-title" style="margin-bottom: 4px;">REQUISITION SLIP
                                        STATUS</div>
                                    <div class="sub-title">SAMPLE PRODUCT:
                                        <strong>{{ $requisitions->no_srs }}</strong></div>
                                </td>
                                <td style="width: 25%;">
                                    <table class="no-border" style="table-layout: auto; width: auto;">
                                        <tr>
                                            {{-- KOLOM JUDUL: Dibuat agar lebarnya pas dengan teksnya --}}
                                            <td style="white-space: nowrap; padding-right: 3px;">
                                        ...
                                        <tr>
                                            <td style="white-space: nowrap; padding-right: 3px;"><strong>Dept</strong>
                                            </td>
                                            <td style="width: 5px;">:</td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if ($correctStatus == 'APPROVED NOT REVIEW')
                                                    <span class="status-indicator status-approved">APPROVED NOT
                                                        REVIEW</span>
                                                @elseif($correctStatus == 'APPROVED WITH REVIEW')
                                                    <span class="status-indicator status-review">APPROVED WITH
                                                        REVIEW</span>
                                            ...
                                            <td>{{ $correctStatus == 'APPROVED NOT REVIEW' ? '-' : $approval->notes ?? '-' }}
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td colspan="5" class="text-center" style="padding: 20px;">No approval
                                            history found.</td>
                                    </tr>
                                @endif
                            </tbody>
